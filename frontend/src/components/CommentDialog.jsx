...
const CommentDialog = ({ open, setOpen }) => {
  ...
  return (
    <Dialog open={open}>
      <DialogContent onInteractOutside={() => setOpen(false)} className="max-w-xl p-0 flex flex-col">
        <div className='flex flex-1'>
          <div className='w-lg'>
            <img
              src={selectedPost?.image}
              alt="post_img"
              className=' max-w-auto max-h-auto object-contain rounded-l-lg'
            />
          </div>
          <div className='w-1/2 flex flex-col justify-between'>
            ...
            <div className='flex-1 overflow-y-auto max-h-96 p-4'>
              {
                comment.map((comment) => <Comments key={comment} comment={comment} />)
              }
            </div>
            <div className='p-4'>
